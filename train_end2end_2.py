...
from models import *
from utility import *
from end_2_end import AEVGG_classifier
...
if __name__ == '__main__':
    ...
    for auto_layer in range(1,5):
        net = AEVGG_classifier(auto_layer).cuda()
        print(net)
        total_epoch = 200
        criterion_ae = nn.MSELoss()
        criterion_cls = nn.CrossEntropyLoss()
        net_parameters = list(net.autoencoder.parameters()) + list(net.vgg16.parameters())
        optimizer = optim.SGD(net_parameters, lr=0.1,momentum=0.9, weight_decay=5e-4)
        ...
        for epoch in range(total_epoch):
            ...
            for i, (data, labels) in enumerate(train_loader):
                ...
                optimizer.zero_grad()
                encoded, decoded, predicts = net(data)
                loss_ae = criterion_ae(data, decoded)
                loss_cls = criterion_cls(predicts, labels)
        ...
        with torch.no_grad():
            for j, data in enumerate(test_loader):
                ...
                encoded, decoded, outputs = net(images)
        ...
        plt.savefig(f'./images/heatmap_test_acc:{n/100}%_AELayer:{auto_layer}_AELoss:{test_loss_ae:.3f}_VGGLoss:{test_loss_cls:.3f}_AE_VGG_end2end_2.png')   
        ...
        plt.legend()
        plt.savefig(f'./images/loss_test_acc:{n/100}%_AELayer:{auto_layer}_AELoss:{test_loss_ae:.3f}_VGGLoss:{test_loss_cls:.3f}_AE_VGG_end2end_2.png')
        ...
        plt.legend()
        plt.savefig(f'./images/acc_test_acc:{n/100}%_AELayer:{auto_layer}_AELoss:{test_loss_ae:.3f}_VGGLoss:{test_loss_cls:.3f}_AE_VGG_end2end_2.png')